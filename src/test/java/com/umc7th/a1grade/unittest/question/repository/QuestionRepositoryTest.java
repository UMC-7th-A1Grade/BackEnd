...
class QuestionRepositoryTest {
  ...
  @Test
  @DisplayName("1회 시도 후 10분 이후 문제 조회, 한 번이라도 조회되면 실패")
  ...
  @Test
  @DisplayName("2회 시도 후 1일 이내 문제는 조회되지 않음, 한 번이라도 조회되면 실패")
  void shouldNotReturnQuestionsWithinOneDay() {
    // Given
    Question targetQuestion = questions.get(0);
    createQuestionLog(targetQuestion, LocalDateTime.now().minusHours(23), true);
    createQuestionLog(targetQuestion, LocalDateTime.now().minusHours(22), true);
  ...
  @Test
  @DisplayName("3회 시도 후 1주일 이후 문제 조회, 한 번이라도 조회되면 실패")
  void shouldReturnQuestionsAfterOneWeek() {
    // Given
    Question targetQuestion = questions.get(0);
    IntStream.range(0, 3)
        .forEach(
            i -> createQuestionLog(targetQuestion, LocalDateTime.now().minusDays(6), i % 2 == 0));
  ...
  @Test
  @DisplayName("4회 시도 후 1달 이후 문제 조회,한 번이라도 조회되면 실패")
  void shouldReturnQuestionsAfterOneMonth() {
    // Given
    Question targetQuestion = questions.get(0);
    IntStream.range(0, 4)
        .forEach(
            i -> createQuestionLog(targetQuestion, LocalDateTime.now().minusDays(29), i % 2 == 0));
  ...
  @Test
  @DisplayName("5회 시도 후 2달 이후 문제 조회, 한 번이라도 조회되면 실패")
  void shouldReturnQuestionsAfterTwoMonths() {
    ...
    IntStream.range(0, 5)
        .forEach(
            i ->
                createQuestionLog(
                    targetQuestion, LocalDateTime.now().minusMonths(2).minusDays(1), i % 2 == 0));
